/*
 * $Id: BeanWizard.java,v 1.2 1999/08/19 05:25:07 multipia Exp $
...
public class BeanWizard{
  ...
  private void initData(){
    ...
    type = new String[num];
    read = new boolean[num];
    write = new boolean[num];
    bound = new boolean[num];
    constrained = new boolean[num];
  ...
  void generateBeanClassCode(){
    try{
      ...
      writer1("/*\n");
      if(!StripString.isNull(classPanel.packName.getText()))
      ...
      for( int i=0; i<proPanel.table.getRowCount(); i++ ){
        ...
        read[i] = proPanel.model.getBooleanValue(i,2);
        write[i] = proPanel.model.getBooleanValue(i,3);
        bound[i] = proPanel.model.getBooleanValue(i,4);
        constrained[i] = proPanel.model.getBooleanValue(i,5);
      ...
      for( int j=0; j<proPanel.table.getRowCount(); j++ ){
        if(StripString.isNull(name[j])) continue;
      ...
      if( classPanel.createEvent.isSelected() ){
        ...
        for(int p=0; p<eventFName.length; p++){
          if(StripString.isNull(eventFName[p])) continue;
