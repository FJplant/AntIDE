/*
 * $Id: AppletDialog.java,v 1.2 1999/08/19 05:25:07 multipia Exp $
 ...
 * @author YunKyung Kim
 */
public class AppletDialog extends JDialog implements ActionListener, KeyListener{
	// CardLayout
	CardLayout topcard;
	...
	JTextField packName = new JTextField(18);
	...
	void jbInit1() throws Exception {
		...
		panel4.setBorder( new BlackTitledBorder( "Option" ) );

		JPanel panel6 = new JPanel();
		panel6.setLayout( new BorderLayout() );
		panel6.add( panel1, "North" );
		panel6.add( panel2, "South" );
		panel6.add( panel4 ); // diff
	...
	void jbInit2() throws Exception {
		table = new ParameterTable( model, 3 );
		...
		JLabel label = new JLabel( "Html Parameter" );
		...
		//panel1.setBorder( new BlackTitledBorder( "HTML Parameter" ) );
	...
	void jbInit3() throws Exception {
		...
		panel4.add( hspace );
 		...
 		panel6.setBorder( new BlackTitledBorder( "HTML Page" ) );
		...
		panel8.add( new JPanel(), BorderLayout.WEST );
		panel8.add( new JPanel(), BorderLayout.EAST );
		//panel8.add( new JPanel(), BorderLayout.NORTH );
		...
		if( cmd.equals( "AddRow" ) ) {
			...
			if( table.getRowCount() > 1 ) {
				...
				if( row == 0 ) {
					...
					table.resetCell( 0, table.getEditingColumn() );
					...
					table.resetCell( row - 1, table.getEditingColumn() );
					//table.setEditingCell();
				}
